...
model_lm <- lm(traindfTrain$MonthlyIncome ~ traindfTrain$Attrition)
salary_value <- predict(model_lm, traindfTrain)
salary_value
diffeerence <- salary_value - traindfTrain$MonthlyIncome 
diffeerence
sqrt(mean(diffeerence^2))
summary(model_lm)


sqrt(mean((traindfTrain$MonthlyIncome-SalSetResult$Salary)^2))

###as.factor(SalSetResult$Salary)
summary(model_lm)




summary(traindfTrain)
